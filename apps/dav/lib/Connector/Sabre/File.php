	...
	public function put($data) {
		...
		try {
			...
			if ($result === false) {
				...
				if ($expected !== "0") {
					throw new Exception($this->l10n->t('Error while copying file to target location (copied bytes: %1$d, expected filesize: %2$d)', [$count, $expected]));
				}
			}
			...
			if (isset($_SERVER['CONTENT_LENGTH']) && $_SERVER['REQUEST_METHOD'] === 'PUT') {
				...
				if ($count !== $expected) {
					throw new BadRequest($this->l10n->t('Expected filesize of %1$d bytes but read (from Nextcloud client) and wrote (to Nextcloud storage) %2$d bytes. Could either be a network problem on the sending side or a problem writing to the storage on the server side.', [$expected, $count]));
				}
			}
		} catch (\Exception $e) {
			$context = [];
		...
		if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] === 'PUT') {
			if (isset($_SERVER['CONTENT_LENGTH'])) {
				...
				if ($bytesWritten !== $expected) {
					...
					throw new BadRequest($this->l10n->t('Expected filesize of %1$d bytes but read (from Nextcloud client) and wrote (to Nextcloud storage) %2$d bytes. Could either be a network problem on the sending side or a problem writing to the storage on the server side.', [$expected, $bytesWritten]));
				}
			}
		}
